...
namespace SGAmod.NPCs.Hellion
{
	...
	public abstract class HellionAttacks
	{
		...
		public static void SpecialAttacks(Player P)
		{
				...
				if (npc.ai[1] > 1000 && npc.ai[1] < 2060)
			{
				...
				if (npc.ai[1] % 6 == 0 && npc.ai[1] < 1850 && npc.ai[1] > 1450)
				{
					...
					int ize = ParadoxMirror.SummonMirror(where + wheretogo * 2f, Vector2.Zero, 50, portaltime, 0f, proj, projectilepattern, 15f, 180);
		...
		public static void BasicAttacks(Player P, int type)
		{
			...
			if (type == 0 || type == 3 || type == 8 || type == 9)
			{
				...
				int proj = ProjectileID.TopazBolt;

					if (npc.ai[0] % 160 < 120 && npc.ai[0] % (type == 8 ? 6 : 4) == 0)
					{
						Vector2 where;
						Vector2 where2 = P.Center - npc.Center;
						where2.Normalize();
						...
						Speedz.Normalize();
						Func<float, bool> projectilepattern = (time) => (time % 10 == 0);
						Func<Vector2, Vector2, float, float, float> projectilefacing = delegate (Vector2 playerpos, Vector2 projpos, float time, float current)
					{
						float val = current;
					...
					if (type == 8 || type == 9)
					{
						...
						hell.manualmovement = 150;
						stick = true;

					}
					int ize = ParadoxMirror.SummonMirror(npc.Center + where, Speedz * (-1f), type>0 ? (type == 8 || type == 9 ? 100 : 60) : 50, 60 + ((type == 3) ? 90 : (type == 8 || type == 9 ? 180 : 0)), angle, proj, projectilepattern, proj==mod.ProjectileType("HellionBeam") ? 2.5f : 15f, proj == ProjectileID.UFOLaser ? 400 : 200, stick);
					//Main.projectile[ize].ai[1] = (npc.ai[0] / 90f) % 1f;
					if (type == 8)
					{
						(Main.projectile[ize].modProjectile as ParadoxMirror).projectilefacing = projectilefacing;
			...
			if (type == 1)
			{
				...
				if (npc.ai[0] % 160 < 160 && npc.ai[0] % 3 == 0)
				{
					...
					float angle = (MathHelper.ToRadians(Main.rand.Next(-10, 10))) + where2.ToRotation();

					int ize = ParadoxMirror.SummonMirror(npc.Center + where, Speedz * Main.rand.NextFloat(-0.5f, 0.5f), 60, 45, angle, ProjectileID.SkyFracture, projectilepattern, 45f + extremeness * 15f, 200);
			...
			if (type == 4)
			{
				...
				float spread = 1f + extremeness;
				int portaltime = 600;
				int proj = ProjectileID.BrainScramblerBolt;
				if (npc.ai[0] % 20 == 0)
				{
					Vector2 where=npc.Center;
					float upordown = 1f;
					...
					Func<float, bool> projectilepattern = (time) => (time>250 && time % 20 == 0);
			...
			if (type == 6)
			{
				...
				if (npc.ai[0] % 10 == 0)
				{
					...
					Func<float, bool> projectilepattern = (time) => (time==105);
		...
		public static void RollSupportAbility(Player P,int reroll)
		{
			...
			rand.Add("Taunt", 0.75);
			if (reroll<1)
			hell.supportabilitycooldown = 60 * Main.rand.Next(20, 30);
		...
		public class Hellion : ModNPC
	{
		...
		public static void HellionManager()
		{
			...
			if (Hellion.instance != null)
			{
				...
				string[] text1 = { "", "Ugh, this is getting annoying", "", "Anger and hate. I summon thee!","","Night time!" };
		...
		public override void PostDraw(SpriteBatch spriteBatch, Color drawColor)
		{
			...
			if (auradraweffect > 0)
			{
				for (int i = 0; i < 720; i += 1)
				{
					...
					spriteBatch.Draw(Main.blackTileTexture, drawPos + thisloc, new Rectangle(0, 0, 64, 64), ((glowingcolors1 * 0.15f) * auradraweffect)* noescapeauravisualsize, 0, new Vector2(32, 32), (new Vector2(1f, 1f) * Main.rand.NextFloat(0.5f, 5f))* noescapeauravisualsize, SpriteEffects.None, 0f);
			...
			for (int rotz = 0; rotz < 5; rotz += 1)
			{
				...
				for (int k = oldRot.Length - 1; k >= 0; k -= 1)
				{
					...
					for (float xx = 0; xx < 1f; xx += 0.20f)
					{
						...
						spriteBatch.Draw(tex, drawPos, new Rectangle(0, timing, tex.Width, (tex.Height - 1) / maxframes), ((Color.Lerp(RainbowDarker, Rainbow, alphaz2) * alphaz) * (appear)) * 0.2f, oldRot[k], drawOrigin, npc.scale, npc.spriteDirection < 0 ? SpriteEffects.None : SpriteEffects.FlipHorizontally, 0f);
					}
				}
			}
			return false;
	...
	public class DPSDrones : ModNPC
	{
		...
		public override void ModifyHitByProjectile(Projectile projectile, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			if ((projectile.penetrate < 1 || projectile.penetrate > 1) && projectile.damage > 20)
			{
