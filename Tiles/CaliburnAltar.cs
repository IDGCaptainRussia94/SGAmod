        ...
        public override bool NewRightClick(int i, int j)
        {
            ...
            for (int z = 0; z < Main.maxPlayers; z += 1)
            {
                if (Main.player[z].active && !Main.player[z].dead)
                {
                    ...
                    {
                        if (Main.player[z].statLife >= 220 && NPC.CountNPCS(mod.NPCType("CaliburnGuardian")) < 1)
                        {
                            if (SGAWorld.downedCaliburnGuardiansPoints > 0)
                            {
                            }
                            else
                            {
                                ...
                                if (Main.myPlayer == z)
                                    Main.NewText("Let see if your worthy...", 50, 150, 50);
                                return true;
                            }
                        }
                        else
                        {
                            if (Main.netMode != 1 && Main.myPlayer == z)
