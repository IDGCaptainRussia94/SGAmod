...
namespace SGAmod.Items.Weapons
{
	...
	public class SurtCharging : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			spriteBatch.Draw(ModContent.GetTexture(this.Texture), (projectile.Center + new Vector2(projectile.direction * 10,-24))-Main.screenPosition, null, lightColor, projectile.rotation, new Vector2(tex.Width / 2f, tex.Height / 2f), new Vector2(1f, 1f), SpriteEffects.None, 0f);
			tex = ModContent.GetTexture("SGAmod/Items/GlowMasks/Surt_Glow");
			spriteBatch.Draw(ModContent.GetTexture(this.Texture), (projectile.Center + new Vector2(projectile.direction * 10, -24)) - Main.screenPosition, null, Color.White, projectile.rotation, new Vector2(tex.Width / 2f, tex.Height / 2f), new Vector2(1f, 1f), SpriteEffects.None, 0f);
				...
				if (projectile.ai[0] > 20f && projectile.ai[0]<1000f)
					{
						...
						if (!((rayloc - 16) - (projectile.position.Y) > 30 || (rayloc - 16) - (projectile.position.Y) < -30))
						{
							...
							int thisoned = Projectile.NewProjectile(pos, new Vector2(projectile.direction * 4, 0), ModContent.ProjectileType<SurtWave>(), (int)(projectile.damage* damagemul)*3, projectile.knockBack * 2f, Main.player[projectile.owner].whoAmI);
