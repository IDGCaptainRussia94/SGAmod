...
namespace SGAmod.Items.Weapons
{
	public class RealityShaper : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			Tooltip.SetDefault("The elements are yours to shape\nRequires a small ammount of mana to swing, Double tab to launch 2 heat waves in succession\nFunctions as both a sword and a staff\nHitting with the blade opens several rifts that launch Sky Fracture Blades into the target\nAfter the swing animation hold left mouse to open a rift to fire very fast cirno bolts and summon portals behind you that summon Hot Rounds!\nThe damage of these are less than the melee damage but are improved by your magic damage multiplier\nIf done after a double tab, you'll summon 2 portals to shoot twice as many projectiles, at double the mana costs");
		...
		public override void Explode()
		{
			...
			if (projectile.timeLeft == timeleftfirerate && projectile.ai[0] > 0)
			{
					...
					if (owner != null && !owner.dead && owner.channel)
				{
					...
					if (everyother == 1)
					{
						int proj = Projectile.NewProjectile(new Vector2(projectile.Center.X, projectile.Center.Y), new Vector2(perturbedSpeed.X, perturbedSpeed.Y)*2f, (int)projectile.ai[0], (int)(projectile.damage* damagescale), projectile.knockBack / 10f, owner.whoAmI);
