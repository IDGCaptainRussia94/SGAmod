...
namespace SGAmod.Items.Weapons
{
	...
	class JarateShurikens : ModItem
	{
		...
		{
			base.SetStaticDefaults();
			DisplayName.SetDefault("Jarate Shurikens");
			Tooltip.SetDefault("Throws a fan of 5 Pissed on Snowballs outwards\nThese inflict Ichored and Sodden\nConsumes 5 of the item per use");
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			{
				Vector2 perturbedSpeed = (new Vector2(speedX, speedY)).RotatedBy(MathHelper.Lerp(-rotation, rotation, (float)i / ((float)numberProjectiles - 1f)));// *player.thrownVelocity;
